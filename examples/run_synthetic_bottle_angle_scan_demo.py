"""Synthetic bottle angle scan demo.

Synthetic solid-cylinder bottle-like smoke check of the baseline
angle scan foundation: synthetic z-axis aligned solid cylinder
(via ``create_synthetic_bottle_body``) -> per-angle parallel ray
grid -> multi-step trace -> detector plane intersection -> detector
grid accumulation -> optical metrics on a relative irradiance
surrogate -> per-angle console summary + max-C99 angle + invariant
check.

This is **not** a real PET bottle STL or a real caustic-risk
result. ``create_synthetic_bottle_body`` returns a simple solid
cylinder approximation: it does not model wall thickness, neck,
shoulder, base curvature, or water volume. The
``interface_sequence`` used here treats the cylinder as a single
PET body (one entry interface, one exit interface), which is a
deliberate simplification for the smoke check; multi-wall and
fluid volumes are out of scope for this task. The reported
"max C99 angle" is the angle with the largest C99 within this
synthetic setup; it is **not** a PET-bottle risk angle. A higher
C99 means a more concentrated caustic (higher local risk), not a
"better" angle.

Run from the repository root:

    python examples/run_synthetic_bottle_angle_scan_demo.py
...
ANGLES_DEGREES = (0.0, 15.0, 30.0, 45.0, 60.0)
BOTTLE_RADIUS = 30.0
BOTTLE_HEIGHT = 120.0
BOTTLE_SECTIONS = 96
RAY_GRID_NX = 11
...
def _print_summary(result: AngleScanResult) -> None:
    print("Synthetic bottle angle scan demo")
    print(
        "Synthetic solid-cylinder bottle-like smoke check; "
        "not a real PET bottle STL or caustic-risk result."
    )

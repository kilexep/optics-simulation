"""Actual STL risk-guided pattern smoke demo.
...
runs a single detailed baseline trace at the caller-supplied
``--baseline-angle`` / ``--baseline-distance``, backtracks the
detector hotspot rays to first-shell-hit ``(u, v)`` coordinates,
builds a contribution map and risk map from those coordinates,
and uses that risk map to drive a Gaussian dimple pattern
restricted to the diagnostic body-region mask of the scaled
shell. Compares the original and risk-guided patterned setups
under the same legacy PET-water angle / detector-distance scan
schedule and reports per-entry thermal-risk deltas.
...
--------------------------------
- This demo backtracks detector hotspot rays to first
  shell-surface hit coordinates.
- The contribution map is a diagnostic map, not a measured
  physical risk field.
- The generated pattern is risk-guided within this surrogate
  pipeline, not optimized.
...
Usage::
    ...
    python examples/run_actual_stl_risk_guided_pattern_demo.py \\
        --mesh data/raw/stl/pet_bottle.stl \\
        --target-height 225.6 --target-diameter 72.1 \\
        --wall-thickness 0.3 --inner-offset-mode auto \\
        --baseline-angle 25 --baseline-distance 160 \\
        --hotspot-top-percent 10 \\
...
def _build_argparser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(
        description=(
            "Actual STL hotspot-backtracked risk-guided pattern "
    ...
    parser.add_argument("--baseline-angle", type=float, default=25.0)
    parser.add_argument(
        "--baseline-distance", type=float, default=160.0,
    )
    parser.add_argument(
...
def _print_static_header() -> None:
    print("Actual STL risk-guided pattern demo")
    print(
        "Actual STL hotspot-backtracked risk-guided pattern smoke "
        "check; not a physical PET-bottle validation."
    )
    print(
        "Note: this demo backtracks detector hotspot rays to first "
        "shell-surface hit coordinates."
    )
    print(
        "Note: the contribution map is a diagnostic map, not a "
        "measured physical risk field."
    )
    print(
        "Note: the generated pattern is risk-guided within this "
        "surrogate pipeline, not optimized."
    )
    print(
...
def _check_invariants(
    *,
    ...
    checks: list[bool] = []
    checks.append(int(detailed.rays.ray_count) > 0)
    checks.append(int(hotspot_result.risk_map.active_count) >= 0)
...
def main(argv: list[str] | None = None) -> int:
    ...
    print(f"Baseline hotspot angle: {float(args.baseline_angle):.4f}")
    print(
        f"Baseline hotspot detector distance: "
        f"{float(args.baseline_distance):.4f}"
    )
    ...
    detailed = run_legacy_pet_water_detailed_trace(
        ...
        angle_degrees=float(args.baseline_angle),
        detector_distance=float(args.baseline_distance),
    ...
    hotspot_result = build_legacy_hotspot_contribution_map(
        detailed,
        top_percent=float(args.hotspot_top_percent),
    )
    print(
        f"Hotspot selected final rays: "
        f"{int(hotspot_result.selected_final_ray_count)}"
    )
    print(
        f"Mapped initial rays: "
        f"{int(hotspot_result.mapped_initial_ray_count)}"
    )
    print(
        f"Contribution nonzero bins: "
        f"{int((hotspot_result.contribution_map.weight_map > 0.0).sum())}"
    )
    print(
        f"Risk active count: "
        f"{int(hotspot_result.risk_map.active_count)}"
    )

    if int(hotspot_result.risk_map.active_count) == 0:
        # No hotspots backtracked; bypass the risk-guided pattern.
        print(
            "Pattern moved vertices: 0 (risk map is empty; "
            "risk-guided pattern skipped)"
        )
        ok = (
            int(body_mask_report.selected_count) > 0
            and int(detailed.rays.ray_count) > 0
        )
        print(f"Invariants: {'PASS' if ok else 'FAIL'}")
        return 0 if ok else 1

    patterned_setup = create_risk_guided_legacy_patterned_pet_water_setup(
        original_setup=original_setup,
        risk_map=hotspot_result.risk_map,
    ...
    comparison = compare_legacy_thermal_risk_scans(
        original_thermal, patterned_thermal,
    )

    o_max_temp = original_thermal.max_temperature_k
    ...
    if o_max_temp is not None and p_max_temp is not None:
        ...
        delta_max_temp = None
    o_thresh = original_thermal.max_threshold_exceeded_count
    ...
    print(
        f"Risk-guided threshold exceeded count: "

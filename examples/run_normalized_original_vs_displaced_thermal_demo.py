"""Normalized original-vs-displaced thermal demo.

Synthetic normalized optical-to-thermal comparison smoke check;
not a physical PET-bottle fire-prevention validation. Wires
together: subdivided synthetic bottle mesh -> per-vertex
normalized cylindrical (u, v) -> synthetic uniform RiskMap ->
Gaussian dimple pattern -> per-vertex displacement amounts ->
displaced mesh copy (mode=inward) -> per-mesh angle-distance
sweep with use_power_weights=True and use_relative_irradiance=True
-> lumped target-heating surrogate per (angle, detector_z) entry
on each mesh -> per-(angle, detector_z) Original vs Displaced
optical and thermal comparison + aggregate "max temperature"
diagnostics for each mesh.

Interpretation notes (also printed at runtime):
- The optical input is a relative irradiance surrogate, **not
  calibrated W/m^2** unless a calibrated incident irradiance is
  supplied.
- The thermal model is a simplified lumped target-heating
  surrogate; **not** pyrolysis, **not** CFD, **not** combustion
  chemistry, **not** ignition validation.
- Delta values are reported for comparison only.
- A negative delta is **not required**.
- This demo does **not** claim fire prevention or PET-bottle
  safety. The threshold is illustrative, not a material
  ignition threshold.
...
- 0-D lumped-capacitance target; constant scalar incident flux;
  no spatial conduction, no spectral absorptivity, no view
  factor, no time-varying source.
...
Run from the repository root:
    ...
    python examples/run_normalized_original_vs_displaced_thermal_demo.py
...
THRESHOLD_TEMP_K = 373.15  # illustrative only
...
def _run_thermal(scan: AngleDistanceScanResult) -> AngleDistanceHeatingScanResult:
    return run_lumped_heating_over_angle_distance_scan(
        scan,
...
def _max_relative(scan: AngleDistanceScanResult) -> float:
    best = 0.0
    for entry in scan.per_result:
        s = entry.irradiance_surrogate
        if s is None:
            continue
        v = float(s.relative_irradiance_map.max())
        if v > best:
            best = v
    return best
...
def _print_summary(
    ...
    original_thermal: AngleDistanceHeatingScanResult,
    displaced_thermal: AngleDistanceHeatingScanResult,
    moved_count: int,
    max_displacement: float,
    vertices_mutated: bool,
    faces_mutated: bool,
) -> None:
    print("Normalized original-vs-displaced thermal demo")
    print(
        "Synthetic normalized optical-to-thermal comparison smoke "
        "check; not a physical PET-bottle fire-prevention validation."
    )
    print(
        "Note: the optical input is a relative irradiance surrogate, "
        "not calibrated W/m^2 unless a calibrated incident "
        "irradiance is supplied."
    )
    print(
        "Note: the thermal model is a simplified lumped "
        "target-heating surrogate; not pyrolysis, not CFD, not "
        "combustion chemistry, not ignition validation."
    )
    print(
    ...
    print(
        "Note: this demo does not claim fire prevention or "
        "PET-bottle safety. The threshold is illustrative, not a "
        "material ignition threshold."
    )
    print(
    ...
    orig_max_rel = _max_relative(original_scan)
    disp_max_rel = _max_relative(displaced_scan)
    print(f"Original max relative irradiance: {orig_max_rel:.4f}")
    print(f"Displaced max relative irradiance: {disp_max_rel:.4f}")
    print(
        f"Delta max relative irradiance: "
        f"{(disp_max_rel - orig_max_rel):+.4f}"
    )
    ...
    if (
        ...
        print(f"Delta max temperature: {d_max_t:+.4f}")
    else:
    ...
    # Find first per-entry threshold-crossing time on each side, for
    # the headline summary.
    def _first_crossing(thermal: AngleDistanceHeatingScanResult):
        for p in thermal.per_result:
            if p.heating_result.time_to_threshold_s is not None:
                return p.heating_result.time_to_threshold_s
        return None

    orig_t_thr = _first_crossing(original_thermal)
    disp_t_thr = _first_crossing(displaced_thermal)
    print(
        f"Original time to threshold: {_fmt_or_none(orig_t_thr, '.4f')}"
    )
    print(
        f"Displaced time to threshold: {_fmt_or_none(disp_t_thr, '.4f')}"
    )

    print(
        "Per-result legend -> Original thermal max: per-entry "
        "lumped-target max temperature on the original mesh. "
        "Displaced thermal max: same on the displaced mesh. Delta "
        "thermal max: Displaced minus Original."
    )
    for op, dp in zip(
        original_thermal.per_result, displaced_thermal.per_result,
    ):
        d_t = (
            float(dp.heating_result.max_temperature_k)
            - float(op.heating_result.max_temperature_k)
        )
        print(
            f"Angle {op.angle_degrees} z={op.detector_z}: "
            f"Original thermal max="
            f"{float(op.heating_result.max_temperature_k):.4f} "
            f"Displaced thermal max="
            f"{float(dp.heating_result.max_temperature_k):.4f} "
            f"Delta thermal max={d_t:+.4f} "
...
def _check_invariants(
    ...
    for thermal in (original_thermal, displaced_thermal):
        for p in thermal.per_result:
            T = p.heating_result.temperature_k
            checks.append(np.isfinite(T).all())
            checks.append(T.size > 0)
            if p.incident_flux_w_m2 > 0.0:
                checks.append(
                    float(p.heating_result.max_temperature_k)
                    >= float(p.heating_result.initial_temp_k) - 1e-12
                )
            if p.heating_result.time_to_threshold_s is not None:
                t = float(p.heating_result.time_to_threshold_s)
                checks.append(0.0 <= t <= float(DURATION_S))
...
def main() -> int:
    ...
    original_thermal = _run_thermal(original_scan)
    displaced_thermal = _run_thermal(displaced_scan)

...
DEMO_SCRIPT = (
    REPO_ROOT
    / "examples"
    / "run_legacy_pet_water_stl_thermal_risk_scan_demo.py"
)
...
def test_demo_runs_against_synthetic_solid_cylinder_stl(
    ...
    assert (
        "Legacy PET-water STL thermal risk scan demo" in result.stdout
    )
    assert (
    ...
    assert "Max threshold exceeded count:" in result.stdout
    assert "Nominal incident irradiance:" in result.stdout
    assert "Threshold temperature:" in result.stdout

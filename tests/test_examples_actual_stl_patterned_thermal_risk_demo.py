...
DEMO_SCRIPT = (
    REPO_ROOT
    / "examples"
    / "run_actual_stl_patterned_thermal_risk_demo.py"
)
...
def test_demo_runs_against_synthetic_solid_cylinder_stl(
    ...
    assert "Actual STL patterned thermal risk demo" in result.stdout
    assert (
    ...
    assert "Patterned max temperature:" in result.stdout

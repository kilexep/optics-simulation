...
DEMO_SCRIPT = (
    REPO_ROOT
    / "examples"
    / "run_actual_stl_self_consistent_subdivided_risk_guided_sweep_demo.py"
)
...
def test_demo_runs_against_synthetic_solid_cylinder_stl(
    ...
    assert (
        "Self-consistent subdivided risk-guided sweep demo"
        in result.stdout
    )
    assert (
        "Self-consistent subdivided actual STL risk-guided "
        "parameter sweep smoke check"
        in result.stdout
    )
    assert "Subdivision iterations:" in result.stdout
    assert "Subdivided readiness:" in result.stdout
    assert "Subdivided selected vertices:" in result.stdout

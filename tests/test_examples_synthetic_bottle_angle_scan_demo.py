...
DEMO_SCRIPT = (
    REPO_ROOT / "examples" / "run_synthetic_bottle_angle_scan_demo.py"
)
...
def test_demo_script_runs_and_invariants_pass() -> None:
    ...
    assert "Synthetic bottle angle scan demo" in result.stdout
    assert "Synthetic solid-cylinder bottle-like smoke check" in result.stdout

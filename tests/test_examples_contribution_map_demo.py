...
DEMO_SCRIPT = REPO_ROOT / "examples" / "run_contribution_map_demo.py"
...
def test_demo_script_runs_and_invariants_pass() -> None:
    ...
    assert "Contribution map demo" in result.stdout
    assert "Synthetic contribution-map smoke check" in result.stdout
    assert "First-hit count:" in result.stdout
    assert "Hotspot selected count:" in result.stdout

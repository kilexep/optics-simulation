...
DEMO_SCRIPT = (
    REPO_ROOT
    / "examples"
    / "run_thermal_map_original_vs_displaced_demo.py"
)
...
def test_demo_script_runs_and_invariants_pass() -> None:
    ...
    assert "Thermal map original-vs-displaced demo" in result.stdout
    assert "Synthetic per-pixel thermal-map surrogate" in result.stdout
    ...
    assert "Original max thermal-map temperature:" in result.stdout
    assert "Displaced max thermal-map temperature:" in result.stdout
    assert "Delta max thermal-map temperature:" in result.stdout
    ...
    assert "Delta top-percent temperature rise:" in result.stdout

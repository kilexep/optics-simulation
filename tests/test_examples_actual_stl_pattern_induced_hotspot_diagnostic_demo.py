...
DEMO_SCRIPT = (
    REPO_ROOT
    / "examples"
    / "run_actual_stl_pattern_induced_hotspot_diagnostic_demo.py"
)
...
def test_demo_runs_against_synthetic_solid_cylinder_stl(
    ...
    assert (
        "Actual STL pattern-induced hotspot diagnostic"
        in result.stdout
    )
    assert "Candidate:" in result.stdout
    assert "Worsened angle:" in result.stdout
    assert "Worsened detector distance:" in result.stdout
    assert "Baseline selected rays:" in result.stdout
    assert "Candidate selected rays:" in result.stdout
    assert "Baseline contribution bins:" in result.stdout
    assert "Candidate contribution bins:" in result.stdout
    assert "Shared contribution bins:" in result.stdout
    assert "Candidate-only contribution bins:" in result.stdout
    assert "Candidate new bin fraction:" in result.stdout

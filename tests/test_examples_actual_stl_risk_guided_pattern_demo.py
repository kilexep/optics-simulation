...
DEMO_SCRIPT = (
    REPO_ROOT
    / "examples"
    / "run_actual_stl_risk_guided_pattern_demo.py"
)
...
def test_demo_runs_against_synthetic_solid_cylinder_stl(
    ...
    assert "Actual STL risk-guided pattern demo" in result.stdout
    assert (
        "Actual STL hotspot-backtracked risk-guided pattern smoke check"
        in result.stdout
    )
    assert "Baseline hotspot angle:" in result.stdout
    assert "Hotspot selected final rays:" in result.stdout
    assert "Mapped initial rays:" in result.stdout
    assert "Contribution nonzero bins:" in result.stdout
    assert "Risk active count:" in result.stdout
    assert "Pattern moved vertices:" in result.stdout
    assert "Original max temperature:" in result.stdout
    assert "Risk-guided max temperature:" in result.stdout

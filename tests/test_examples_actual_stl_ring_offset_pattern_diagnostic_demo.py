...
DEMO_SCRIPT = (
    REPO_ROOT
    / "examples"
    / "run_actual_stl_ring_offset_pattern_diagnostic_demo.py"
)
...
def test_demo_runs_against_synthetic_solid_cylinder_stl(
    ...
    assert (
        "Actual STL ring-offset pattern diagnostic demo"
        in result.stdout
    )
    assert (
    ...
    assert "Centered risk-guided pattern" in result.stdout
    assert "Ring-offset risk-guided pattern" in result.stdout
    assert "Ring inner radius:" in result.stdout
    assert "Ring outer radius:" in result.stdout
    assert "Source risk active count:" in result.stdout
    assert "Ring risk active count:" in result.stdout
    assert "Centered delta max temperature:" in result.stdout
    assert "Ring delta max temperature:" in result.stdout
    assert "Centered candidate-only bins:" in result.stdout
    assert "Ring candidate-only bins:" in result.stdout
